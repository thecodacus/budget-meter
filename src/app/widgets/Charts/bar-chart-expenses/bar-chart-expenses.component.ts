...
import { ITransaction, StoreService } from 'src/app/services/store.service';
...
export class BarChartExpensesComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.expenses$ = this.store.getTotalExpence(new Date(`${new Date().getFullYear()}-${new Date().getMonth() - 5}-01`), new Date())
